...
public class WorldRep : MonoBehaviour
{
    ...
    public static float TerrainYHeightAtPosition(Vector3 pos)
    {
        ...
        if (Physics.Raycast(pos + Vector3.up * 500f, Vector3.down, out RaycastHit hit, 1000f, terrainLayerMask))
        {
            return hit.point.y;
        }
        else
        {
            Debug.LogWarning("No terrain found at position " + pos);
            return 0f;
        }
    }
    ...
    {
        float Epsilon_position = 0.1f;
        float groundHeight = TerrainYHeightAtPosition(col.transform.position);
        float bottomOfCollider = col.bounds.min.y;

        return Mathf.Abs(groundHeight - bottomOfCollider) < Epsilon_position;
    }
}
